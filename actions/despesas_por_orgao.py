...
def fetch_data(pagina, orgao_superior, ano):
    ...
    headers = {
        'chave-api-dados': 'e6152c1558b273b4666d27d60ed32f3f',  # Usando a chave de API obtida da variável de ambiente
...
def fetch_and_save_data(ano):
    # Determina o caminho da pasta 'jsons' no diretório onde o script está localizado
    jsons_dir = os.path.join(os.path.dirname(__file__), 'json')

    # Cria a pasta 'jsons' caso não exista
    ...
    for orgao, codigo in ORGAOS_SUPERIORES.items():
        ...
        while True:
            ...
            for item in data:
                item_serialized = json.dumps(item, ensure_ascii=False)  # Serializa o item para string
                if item_serialized not in seen:  # Se ainda não foi adicionado
                    all_data.append(item)
                    seen.add(item_serialized)  # Adiciona ao conjunto seen
